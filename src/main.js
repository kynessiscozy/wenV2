...
(function(){
 const T={
  ...
  answerbook:{k:'灵感与娱乐',icon:'?',title:'答案之书',desc:'把一个问题写下来，翻开一句简短答案，作为整理思路的提示。它不是预测，也不能替代你的判断。',fields:[['question','你的问题','textarea','例如：我现在适合开始这件事吗？']]},
 ...
 function run(type){const d=window._ctx||window._baziData||{},wx=d.wx||{},scoreBase=d.cs||60;
  ...
  if(type==='answerbook'){const answers=['可以。先从最小、可逆的一步开始。','再等等，先把关键信息补齐。','答案不在猜测里，去问清楚、看事实。','如果这件事让你持续抗拒，先停下来照顾自己。','换个角度再看一次，也许会发现新的选项。','现在不必追求完美，先完成一个小版本。','相信你的直觉，但给它一个现实验证的机会。','把问题拆小，今天只处理其中一部分。','是的，但要设好边界和截止时间。','暂时不要，保留选择比仓促承诺更重要。'];const q=val('question')||'我现在应该怎么做？';const answer=answers[Math.floor(Math.random()*answers.length)];result('答案之书','问题：<strong>'+esc(q)+'</strong>。<div class="answer-book-card"><div class="answer-book-mark">✦</div><div class="answer-book-answer">'+answer+'</div><div class="answer-book-note">把这句话当作一个新的视角，再结合事实、感受和实际条件做决定。</div></div>');return}
